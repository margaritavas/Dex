...
//The user must have enough tokens deposited such that token balance >= sell order amount
    ...
    let dex = await Dex.deployed();
    let link = await Link.deployed();
    truffleAssert.reverts(dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 10, 5));
...
//The BUY order book should be ordered on price from highest to lowest starting at index 0
    ...
    for (let i=0, i < orderbook.length - 1, i++){
        assert(orderbook[i] >= orderbook[i+1].price, "not right order in buy book")
    }
})
...
it("The BUY order book should be ordered on price from highest to lowest starting at index 0", async() =>{
    ...
    for (let i=0, i < orderbook.length - 1, i++){
        assert(orderbook[i] <= orderbook[i+1].price, "not right order in sell book")
    }
})